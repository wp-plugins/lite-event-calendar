...
class dpProEventCalendar_wpress_display {
	...
	function dpProEventCalendar_wpress_display($id, $type, $limit, $widget, $limit_description, $category, $author, $get = "", $event_id = "", $event = "", $columns = "", $from = "") {
		self::$id_calendar = $id;
		self::$type = $type;
		self::$limit = $limit;
		self::$widget = $widget;
	...
	function return_dpProEventCalendar() {
		...
		$dpProEventCalendar_class = new DpProEventCalendar( false, $id, null, null, $widget, $category, $event_id, $author, $event, $columns, $from );
...
function dpProEventCalendar_simple_shortcode($atts) {
	...
	if ( !is_admin() ){ 
		...
		wp_enqueue_script( 'placeholder', dpProEventCalendar_plugin_url( 'js/jquery.placeholder.js' ),
			array('jquery'), DP_LITE_EVENT_CALENDAR_VER, false); 
	...
	$dpProEventCalendar_wpress_display = new dpProEventCalendar_wpress_display($id, $type, $limit, $widget, $limit_description, $category, $author, $get, $event_id, $event, $columns, $from);
...
class DpProEventCalendar_Widget extends WP_Widget {
	...
	public function widget($args, $instance) {
		global $wpdb, $table_prefix;
		$table_name = $table_prefix.DP_LITE_EVENT_CALENDAR_TABLE_EVENTS;
		...
		echo $before_widget;
			...
			echo do_shortcode('[dpProEventCalendar id='.$calendar.' widget=1 category="'.$category.'" author="'.$author.'"]');
		echo $after_widget;
...
class DpProEventCalendar_UpcomingEventsWidget extends WP_Widget {
	function __construct() {
		$params = array(
			...
			'name' => 'DP Lite Event Calendar - Upcoming Events'
		);
	...
	public function widget($args, $instance) {
		...
		echo $before_widget;
			if(!empty($title))
				echo $before_title . $title . $after_title;
			echo '<p>'. $description. '</p>';
			echo do_shortcode('[dpProEventCalendar id='.$calendar.' type="'.$type.'" category="'.$category.'" limit="'.$events_count.'" limit_description="'.$limit_description.'" columns="1"]');
		echo $after_widget;
...
class DpProEventCalendar_TodayEventsWidget extends WP_Widget {
	function __construct() {
		$params = array(
			...
			'name' => 'DP Lite Event Calendar - Today\'s Events'
		);
...
function dpProEventCalendar_admin_scripts($force = false) {
	...
	if ( is_admin() ){ // admin actions
		...
		or 'dpProEventCalendar-pro' == $_GET['page'] ))  ) {
...
function dpProEventCalendar_admin_head() {
	...
	<?php
	   ...
	   // Calendars page only
		if ( isset($_GET['page']) && ('dpProEventCalendar-admin' == $_GET['page'] or 'dpProEventCalendar-pro' == $_GET['page']) ) {
		?>
			<script type="text/javascript">
			// <![CDATA[
